...
export default function CuentasCobrar() {
    ...
    const handleCrearCuenta = async () => {
        if (!nuevaCuenta.clienteId || !nuevaCuenta.monto) {
            alert('Por favor complete el cliente y monto');
            return;
        }

        try {
            await api.post('/cuentas-cobrar', {
                clienteId: parseInt(nuevaCuenta.clienteId),
            ...
            fetchData();
            setShowModal(false);
            setNuevaCuenta({ clienteId: '', descripcion: '', monto: '', fechaVencimiento: '' });
        } catch (error) {
    ...
    const handleRegistrarPago = async () => {
        ...
        try {
            await api.post(`/clientes/${cuentaActiva.clienteId}/abono-fifo`, {
    ...
    return (
        <div id="cuentas-root" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            {/* Modal Nueva Cuenta */}
            <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Nueva Cuenta por Cobrar">
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', minWidth: '400px' }}>
                    <div>
                        <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, marginBottom: '6px' }}>Cliente *</label>
                        <select
                            value={nuevaCuenta.clienteId}
                            onChange={(e) => setNuevaCuenta(prev => ({ ...prev, clienteId: e.target.value }))}
